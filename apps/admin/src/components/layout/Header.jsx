        ...
        <div className="relative" ref={menuRef}>
          ...
          >
            <div className="hidden sm:block text-right">
              <p className="text-[13px] font-bold text-on-surface tracking-tight leading-none mb-1 max-w-[140px] truncate">{displayName}</p>
              <p className="text-[9px] text-primary font-bold tracking-widest uppercase">{roleLabel}</p>
            </div>
            <div className="w-8 h-8 md:w-9 md:h-9 rounded-full bg-primary text-white flex items-center justify-center overflow-hidden ring-2 ring-surface-container-highest font-bold text-[12px] uppercase shadow-sm">
              {admin?.avatarUrl ? (
          ...
          {menuOpen && (
            <div className="absolute right-0 top-12 z-50 w-64 bg-white rounded-xl shadow-2xl border border-outline-variant/20 overflow-hidden animate-[fadeIn_0.15s_ease-out]">
              <div className="px-4 py-3 bg-gradient-to-br from-[#06201B] to-[#0a3029] text-white">
                <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-emerald-300">Signed in as</p>
                <p className="text-[14px] font-bold mt-0.5 truncate">{displayName}</p>
                <p className="text-[11px] text-emerald-100/70 truncate">{admin?.email}</p>
              </div>
              <div className="py-1">
...
const MenuItem = ({ icon, tone, onClick, children }) => (
  <button
    onClick={onClick}
    className={`w-full flex items-center gap-3 px-4 py-2.5 text-[13px] font-semibold text-left transition-colors ${
