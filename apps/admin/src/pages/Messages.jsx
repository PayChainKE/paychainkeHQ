        ...
        <aside className="hidden lg:flex flex-col w-56 bg-[#0a1f1a] text-white border-r border-emerald-900/30">
          <div className="px-5 py-5 border-b border-emerald-900/30">
            <div className="flex items-center gap-2">
              <span className="material-symbols-outlined text-emerald-300 text-[18px]" style={{ fontVariationSettings: "'FILL' 1" }}>headset_mic</span>
              <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-emerald-300">Support Console</p>
            </div>
          ...
          <div className="px-3 py-4 space-y-1 flex-1 overflow-y-auto">
            ...
            <p className="px-3 text-[10px] font-bold uppercase tracking-widest text-emerald-300/40 mb-1">Status</p>
            ...
            <div className="h-px bg-emerald-900/30 my-3"></div>
            <p className="px-3 text-[10px] font-bold uppercase tracking-widest text-emerald-300/40 mb-1">Type</p>
            <select
              value={typeFilter}
              onChange={(e) => setTypeFilter(e.target.value)}
              className="w-full bg-[#06201B] border border-emerald-900/40 rounded-lg text-white text-[12px] px-3 py-2 focus:outline-none focus:border-emerald-400"
            >
          ...
          <div className="px-5 py-4 border-t border-emerald-900/30">
            <p className="text-[10px] font-bold uppercase tracking-widest text-emerald-300/50 mb-1">Last refresh</p>
            <p className="text-[11px] text-emerald-100/60">{relativeTime(Date.now())}</p>
            <button onClick={fetchMessages} className="mt-2 w-full px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white text-[11px] font-bold uppercase tracking-widest rounded-lg transition-colors">
              Refresh
            </button>
          </div>
        </aside>
          ...
          <div className="px-4 pt-4 pb-3 border-b border-outline-variant/10 space-y-3 flex-shrink-0">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2 bg-surface-container-low border-transparent focus:border-primary focus:ring-0 rounded-lg text-[13px] text-on-surface"
              />
            </div>
  ...
  return (
    <>
      ...
      <div className="bg-white border-b border-outline-variant/10 px-4 md:px-6 py-3 flex items-center gap-3">
        ...
        <div className={`w-10 h-10 rounded-full flex-shrink-0 flex items-center justify-center font-bold text-[12px] uppercase bg-primary text-white shadow-sm`}>
          {initials(m.name)}
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <p className="text-[15px] font-bold text-on-surface tracking-tight truncate">{m.name}</p>
            <span className={`inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-bold uppercase tracking-widest border ${typeMeta.color}`}>{typeMeta.label}</span>
          </div>
          <p className="text-[12px] text-on-surface-variant/60 truncate">{m.email} · {m.phone || 'no phone'}</p>
        </div>
        <div className="flex items-center gap-1 flex-shrink-0">
          <select
            value={m.status || 'open'}
            onChange={(e) => onChangeStatus(e.target.value)}
            className={`text-[11px] font-bold uppercase tracking-widest border rounded-lg px-2 py-1 outline-none cursor-pointer ${st.pill}`}
          >
          ...
          <button onClick={onTogglePriority} className={`p-2 rounded-lg transition-colors ${m.priority ? 'text-amber-500 bg-amber-50' : 'text-on-surface-variant/30 hover:bg-surface-container-low'}`} title={m.priority ? 'Remove flag' : 'Flag for follow-up'}>
            <span className="material-symbols-outlined text-[20px]" style={{ fontVariationSettings: m.priority ? "'FILL' 1" : '' }}>flag</span>
          </button>
          <button onClick={onToggleUnread} className="p-2 rounded-lg text-on-surface-variant/40 hover:bg-surface-container-low transition-colors" title={m.isRead ? 'Mark unread' : 'Mark read'}>
            <span className="material-symbols-outlined text-[20px]">{m.isRead ? 'mark_email_unread' : 'mark_email_read'}</span>
          </button>
        ...
        <div className="max-w-3xl mx-auto px-4 md:px-8 py-6 space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-outline-variant/10 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-outline-variant/10 flex items-start justify-between gap-3 bg-surface-container-low/30">
              <div>
                <h3 className="text-[17px] font-bold text-on-surface tracking-tight">{m.subject}</h3>
                <p className="text-[11px] text-on-surface-variant/60 mt-0.5">From <strong>{m.name}</strong> · {fmtDate(m.createdAt)}</p>
              </div>
            </div>
            <div className="p-5 text-[14px] leading-[1.75] text-on-surface whitespace-pre-wrap">{m.message}</div>
            {m.referralSource && (
              <div className="px-5 pb-4 text-[11px] text-on-surface-variant/50">Referred via: <span className="font-bold">{m.referralSource}</span></div>
            )}
          </div>
              ...
              <div className="flex items-center justify-between mt-2">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="px-4 py-2 rounded-lg bg-primary text-white text-[11px] font-bold uppercase tracking-widest hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center gap-2"
                  >
...
const RailLink = ({ active, onClick, icon, count, tone, children }) => (
  ...
  >
    ...
    {count != null && count > 0 && (
      <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${tone === 'red' ? 'bg-red-500/30 text-red-200' : tone === 'emerald' ? 'bg-emerald-500/30 text-emerald-200' : 'bg-white/10 text-white/80'}`}>
        {count}
      </span>
    )}
  </button>
