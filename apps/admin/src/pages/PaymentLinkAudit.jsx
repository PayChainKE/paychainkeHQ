...
/**
 * Two complementary views of merchant-generated Payment Links, across every
 * merchant:
 *
 * - "Paid" (default): every customer payment that came in through a link —
 *   a Transaction Audit view scoped to `paymentLinkId != null` (see
 *   backend/models/Transaction.js's `paymentLinkId` field, stamped in
 *   mpesaController.js when an STK Push that settles a PaymentLink
 *   resolves). Reuses the same GET /api/admin/transaction-audit search
 *   endpoint as the general Transaction Audit page (with
 *   `viaPaymentLink=true`), so filtering, pagination, and the detail
 *   drawer's STK/SMS correlation all work exactly the same way — just
 *   pre-scoped to payment-link traffic. Permanent record (Transaction rows
 *   never expire), but only covers links that actually got paid.
 *
 * - "Active" (LivePaymentLinksTable below): every PaymentLink document that
 *   currently exists — generated but not yet paid, alongside any not-yet-
 *   expired paid ones. A link nobody ever paid never becomes a Transaction,
 *   so without this tab it was invisible in admin entirely. Necessarily a
 *   live snapshot, not a permanent record — models/PaymentLink.js's TTL
 *   index physically deletes each doc ~1 minute after its own expiresAt,
 *   and the model's own pre-find/pre-findOne hooks filter out anything
 *   already past expiresAt even before that deletion runs, so an
 *   expired-unpaid link can never be shown after the fact.
 */
const PaymentLinkAudit = () => {
  const [tab, setTab] = useState('paid'); // 'paid' | 'active'
  ...
  return (
    <Layout>
      <div className="space-y-8 pb-12">
          ...
          <div className="relative">
            ...
            <p className="text-xs md:text-sm text-indigo-100/60 mt-2 max-w-2xl font-body">
              {tab === 'paid'
                ...
                : "Every link that's currently outstanding — generated by a merchant but not yet paid or expired. Links that never get paid never show up under Paid, since no Transaction is ever created for them."}
            </p>
          </div>
        </div>

        <div className="flex bg-surface-container-low p-1 rounded-xl w-fit border border-outline-variant/10">
          {[
            { id: 'paid', label: 'Paid' },
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-widest transition-all ${
                tab === t.id ? 'bg-white text-on-surface shadow-sm' : 'text-on-surface-variant/60 hover:text-on-surface'
              }`}
            >
              {t.label}
            </button>
        ...
        {tab === 'active' && <LivePaymentLinksTable />}
...
const LINK_STATUS_META = {
  active: { label: 'Active',  pill: 'bg-amber-50 text-amber-700 border-amber-200',       dot: 'bg-amber-500 animate-pulse' },
  paid:   { label: 'Paid',    pill: 'bg-emerald-50 text-emerald-700 border-emerald-200', dot: 'bg-emerald-500' },
};

// Origin of a link — mirrors what mints a PaymentLink across the codebase:
// an Invoice's own "Get Payment Link" (invoiceId set), a Checkout Page cart
// checkout (checkoutPageId set), or the plain ad-hoc case (Wallet's Request
// Money, PayAccountPage) where neither is set.
function linkOrigin(l) {
  if (l.invoiceId) return 'Invoice';
  if (l.checkoutPageId) return 'Checkout Page';
  return 'Ad-hoc';
}

// ── "Active / Pending" tab — GET /api/admin/payment-links, live
//    PaymentLink documents (not yet paid or expired) across every
//    merchant. See PaymentLinkAudit's own doc comment for why this can
//    only ever be a live snapshot, not a permanent history. ───────────────
const LivePaymentLinksTable = () => {
  const [rows, setRows] = useState([]);
  const [total, setTotal] = useState(0);
  const [page, setPage] = useState(1);
  const [loading, setLoading] = useState(true);
  ...
  const fetchRows = useCallback(async () => {
    ...
    setError('');
    try {
      const res = await api.get('/api/admin/payment-links', { params });
      if (res.data?.success) {
        setRows(res.data.data || []);
        setTotal(res.data.pagination?.total || 0);
      } else {
        setError(res.data?.error || 'Could not load active payment links.');
      }
    } catch (e) {
      setError(e?.response?.data?.error || 'Could not load active payment links.');
    } finally {
      setLoading(false);
    }
  }, [params]);
      ...
      <div className="bg-surface-container-lowest rounded-xl border border-outline-variant/20 p-3 md:p-4 flex flex-col lg:flex-row gap-3 items-stretch lg:items-center">
        <div className="relative flex-1 min-w-0">
          ...
          <input
            value={searchInput}
            onChange={(e) => setSearchInput(e.target.value)}
            placeholder="Link ID, merchant name/email…"
            className="w-full pl-10 pr-3 py-2 bg-surface-container-low border-transparent focus:border-secondary focus:ring-0 rounded-lg text-sm text-on-surface placeholder:text-on-surface-variant/40"
          />
        </div>
        <select value={status} onChange={(e) => { setStatus(e.target.value); setPage(1); }} className="text-xs font-semibold bg-surface-container-low border border-outline-variant/20 rounded-lg px-3 py-2 text-on-surface focus:border-primary focus:ring-0">
          ...
          <table className="w-full text-left font-body">
            ...
            <tbody className="text-xs">
              {loading ? (
                ...
                <tr><td colSpan="7" className="py-10 text-center text-error text-sm">{error}</td></tr>
              ) : rows.length === 0 ? (
                <tr><td colSpan="7" className="py-10 text-center text-on-surface-variant/40 text-sm">No links are currently outstanding.</td></tr>
              ) : rows.map((l) => {
                const sm = LINK_STATUS_META[l.status] || LINK_STATUS_META.active;
                return (
                  <tr key={l._id} className="hover:bg-secondary-container/5 transition-colors">
                    <td className="px-3 py-2 border-b border-outline-variant/5">
                      <span className="font-mono text-2xs font-bold text-on-surface bg-surface-container-low px-2 py-1 rounded">
                        {l.linkId?.length > 20 ? `${l.linkId.slice(0, 16)}…` : l.linkId || '—'}
                      </span>
                    </td>
                    <td className="px-3 py-2 border-b border-outline-variant/5">
                      {l.merchant ? (
                        <p className="font-bold text-on-surface tracking-tight truncate max-w-[160px]">{l.merchant.businessName}</p>
                      ) : <span className="text-on-surface-variant/40 italic">Deleted Merchant</span>}
                    </td>
                    <td className="px-3 py-2 border-b border-outline-variant/5 text-on-surface-variant/70">{linkOrigin(l)}</td>
                    <td className="px-3 py-2 border-b border-outline-variant/5 text-right font-bold text-on-surface tabular-nums">{fmtKES(l.amount)}</td>
                    <td className="px-3 py-2 border-b border-outline-variant/5 text-on-surface-variant/70 whitespace-nowrap">{fmtTime(l.createdAt)}</td>
                    <td className="px-3 py-2 border-b border-outline-variant/5 text-on-surface-variant/70 whitespace-nowrap">{fmtTime(l.expiresAt)}</td>
                    <td className="px-3 py-2 border-b border-outline-variant/5">
                      <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-2xs font-bold uppercase tracking-widest border ${sm.pill}`}>
                        <span className={`w-1.5 h-1.5 rounded-full ${sm.dot}`}></span>
                        {sm.label}
