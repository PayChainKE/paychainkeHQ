...
// counties — mirrors backend/config/kenyaCountyAreas.js exactly. Baked in
// as a static literal (not fetched) for the same reason web's Login.jsx
// does: it's a fixed, rarely-changing ~340-entry list, unlike ward data
// (~1,450 entries) which is fetched at runtime instead — see
// kenyaLocations.ts. Keys must exactly match KENYAN_COUNTIES in
// pages/Login.tsx. Keep in sync with the backend file if it ever changes.
export const KENYA_COUNTY_AREAS: Record<string, string[]> = {
