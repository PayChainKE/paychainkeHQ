...
const NewsletterSignup: React.FC = () => {
  ...
  const onSubscribe = async (e: React.FormEvent) => {
    ...
    try {
      const response = await api.post('/api/newsletter/subscribe', { email, name: name.trim() || undefined });
    ...
    <div className="w-full max-w-md">
      <form onSubmit={onSubscribe} className="flex flex-col gap-3">
        <input
          ...
          maxLength={100}
          className="w-full bg-emerald-50/10 border border-emerald-50/20 rounded-xl px-4 py-3 text-sm text-emerald-50 focus:ring-2 focus:ring-primary outline-none transition-all placeholder:text-emerald-50/40"
        />
