...
// This is the SAME professional Cash Advance application form as
// merchant-dashboard/src/pages/CashAdvance.jsx — same steps, same
// eligibility/status states, same validation. The only difference is the
// data source: by default (no real login) it runs on realistic mock data
// so any demo visitor can explore the full flow with zero friction, same
// as Wallet.jsx/InflationShield.jsx. If the demo merchant account is
// connected (see StellarConnectPanel), it switches to the real trust
// score, real application history, and a real submission to
// /api/cash-advance/apply — genuinely the same backend merchant-dashboard
// uses, not a re-implementation that could drift from it.
...
const MOCK_TRUST_DATA = { eligibleForAdvance: true, current: 78 }
...
// Small, non-intrusive prompt to swap this page over to real backend data —
// same pattern as the "Connect real testnet wallet" link on Wallet.jsx.
function ConnectRealDataPrompt() {
  ...
  return (
    <div className="mb-6 lg:mb-8">
      <button
        type="button"
        onClick={() => setShow((v) => !v)}
        className="text-[10px] font-bold uppercase tracking-widest text-emerald-700 hover:text-emerald-800 underline underline-offset-2"
      >
        {show ? 'Hide' : 'This is a live demo — connect the real demo merchant account'}
      </button>
      {show && (
        <div className="mt-4 animate-fade-in-up">
          <StellarConnectPanel
            title="Connect the demo merchant account"
            subtitle="Sign in to check a real trust score and submit a genuine cash advance application against PayChain's live backend."
          />
        </div>
      )}
    </div>
...
export default function CashAdvance() {
  ...
  const isConnected = !!realMerchant
  ...
  // Only a real, connected merchant can have this feature flag turned off —
  // the mock walkthrough is always explorable, same as every other mock
  // page in this app.
  const formDisabled = isConnected && !!(realMerchant?.features && realMerchant.features.cashAdvanceForm === false)

  useEffect(() => {
    if (realLoading) return

    if (!isConnected) {
      // Self-contained mock experience — no network calls, same philosophy
      // as MerchantAuthContext's mock login()/setNewPassword().
      setTrustData(MOCK_TRUST_DATA)
      setApplications([])
      setIsLoading(false)
      return
    }

    if (contactPhone === '' && realMerchant.phone) setContactPhone(realMerchant.phone)
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (isConnected) {
        ...
        // Mock submission — same simulated-delay convention as
        // MerchantAuthContext's login()/setNewPassword().
    ...
    return (
      <MerchantLayout title="Cash Advance">
        <div className="px-1 lg:px-0 max-w-4xl mx-auto w-full space-y-8 lg:space-y-12">
          <UnavailableNotice />
          <PageHeader />
          {!isConnected && <ConnectRealDataPrompt />}
